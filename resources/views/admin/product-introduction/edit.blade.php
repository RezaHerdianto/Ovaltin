            ...
            <form action="{{ route('admin.product-introduction.update', $introduction->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="2" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm @error('description') border-red-500 @enderror">{{ old('description', $introduction->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="5" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm @error('content') border-red-500 @enderror">{{ old('content', $introduction->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="feature_1_description" id="feature_1_description" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm">{{ old('feature_1_description', $introduction->feature_1_description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="feature_2_description" id="feature_2_description" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm">{{ old('feature_2_description', $introduction->feature_2_description) }}</textarea>
                        </div>
                    </div>
                </div>
