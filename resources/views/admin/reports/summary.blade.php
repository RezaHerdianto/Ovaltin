...
<body>
    ...
    <div class="meta">
        ...
        Periode: {{ $generatedAt->copy()->startOfMonth()->format('d/m/Y') }} - {{ $generatedAt->copy()->endOfMonth()->format('d/m/Y') }}
    </div>
    ...
    <h2>2. Statistik Pengguna Tahunan ({{ $reportYear }})</h2>
    @if ($userTrendChart)
    ...
    <h2>3. Distribusi Status Produk</h2>
    ...
    <h2>5. Pertumbuhan Testimoni Tahunan ({{ $reportYear }})</h2>
    ...
    <h2>6. Informasi Kontak Utama</h2>
    @if ($activeContact)
        <table>
            <tbody>
                <tr>
