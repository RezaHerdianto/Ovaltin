        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            <div class="px-4 py-5 sm:p-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <!-- Product Image -->
                    <div>
                        @if($strawberryProduct->image_url)
                            <img class="w-full h-64 object-cover rounded-lg" src="{{ $strawberryProduct->image_url }}" alt="{{ $strawberryProduct->name }}">
                        @else
                            <div class="w-full h-64 bg-red-100 rounded-lg flex items-center justify-center">
                                <svg class="w-24 h-24 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div>
                            ...
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 font-semibold text-red-600">{{ $strawberryProduct->formatted_price }}</dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Grade Kualitas</dt>
                                    <dd class="mt-1">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            @if($strawberryProduct->quality_grade === 'Premium') bg-purple-100 text-purple-800
                                            @elseif($strawberryProduct->quality_grade === 'Grade A') bg-green-100 text-green-800
                                            @elseif($strawberryProduct->quality_grade === 'Grade B') bg-yellow-100 text-yellow-800
                                            @else bg-gray-100 text-gray-800
                                            @endif">
                                            {{ $strawberryProduct->quality_grade }}
                                        </span>
                                    </dd>
                                </div>
                                <div>
